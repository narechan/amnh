...
if ($mode eq "PAUP"){
    ...
    $cfiobj->sub_matrix ($sample,
			 ...
			 $datatype,
                         "$outdir/cmds");
    $cfiobj->run_tnt ("$outdir/cmds/$sample.$number.nex",
		      $outdir);
    
    # clean out TNT junk and punctuate for newick
    # extract last tree string as that's the consensus
    open (T, "$outdir/trees/$sample.$number.tre");
    my $tstring;
    while (my $line = <T>){
	chomp $line;
	
	next if ($line =~m/^tread/);
	next if ($line =~m/^proc/);
    ...
    open (N, ">$outdir/trees/$sample.$number.tre.new");
    print N "$tstring\n";
    close (N);

    `rm $outdir/trees/$sample.$number.tre`;
    
    $cfiobj->tree_converter ("$outdir/trees/$sample.$number.tre.new",
			     "newick",
                             "$outdir/trees/$sample.$number.tre",
			     "nexus");

    `rm $outdir/trees/$sample.$number.tre.new`;
}
