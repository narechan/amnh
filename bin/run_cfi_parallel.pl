...
if ($submit == 2){
    ...
    foreach my $i (@expts){
	print SHELL "cfi_run.pl -m $matrixfile -c $configfile -o $outdir -e $outdir/cmds/\$SGE_TASK_ID.$i.cmds\n";
    }
...
# create a shell script for every samp and submit expts as an array (multiple arrays of parallel jobs)
	...
	print SHELL "date\n";
	print SHELL "source ~/.bashrc\n";
	print SHELL "cfi_run.pl -m $matrixfile -c $configfile -o $outdir -e $outdir/cmds/$i.\$SGE_TASK_ID.cmds\n";
	print SHELL "hostname\n";
	print SHELL "date\n";
	close (SHELL);
...
# good for if you have a step size
    ...
    for (my $i = 1; $i <= $samps; $i++){
	    ...
	    print SHELL "date\n";
	    print SHELL "source ~/.bashrc\n";
	    print SHELL "cfi_run.pl -m $matrixfile -c $configfile -o $outdir -e $outdir/cmds/$i.$j.cmds\n";
	    print SHELL "hostname\n";
	    print SHELL "date\n";
	    close (SHELL);
...
# do a hybrid submit
    ...
    for (my $i = 1; $i <= $samps; $i++){
        ...
        print SHELL2 "cfi_run.pl -m $matrixfile -c $configfile -o $outdir -e $outdir/cmds/$i.\$SGE_TASK_ID.cmds\n";
...
print T "cfi_topos.pl -m $matrixfile -c $configfile -o $outdir\n";
print T "hostname\n";
print T "date\n";
close (T);
...
until ($bail > 0){
	...
	if ($treefile){
	    ...
	    print PAR "cfi_parse.pl -m $matrixfile -c $configfile -o $outdir -i \$SGE_TASK_ID -d $outdir/topology.distribution\n";# -p\n";
	    print PAR "hostname\n";
	    print PAR "date\n";
	    close (PAR);
