...
for (my $i = 1; $i <= $reps; $i++){
	...
	foreach my $reffile (@readfiles){
		...
		if ($clean){
		    `insid.pl -p $procs -x -c $configfile -s $outdir/reps/$i/qry_reads/$qryfile -r $outdir/reps/$i/ref_reads/$reffile/$basecountR.fa -o $outdir/reps/$i/results/$basecountR/$qryfile-$reffile`;
		}
		else {
		    `insid.pl -p $procs -c $configfile -s $outdir/reps/$i/qry_reads/$qryfile -r $outdir/reps/$i/ref_reads/$reffile/$basecountR.fa -o $outdir/reps/$i/results/$basecountR/$qryfile-$reffile`;
		}
