...
my ($help, $taxanum, $genenum, $outdir, $genelen, $rates, $treenum, $fracmissing);
# genenum correspond to the number of trees and genes to build
...
GetOptions(
    ...
    'r|rates=s'     => \$rates, # as in -r 0.5,1.5
	   ) or pod2usage;
pod2usage if $help;
...
# initialize a TreeIO writer and write the random tree
...
# handle the rates
my ($rate1, $rate2) = split (/\,/, $rates);
my @rates;
push (@rates, $rate1, $rate2);

# build the seq-gen file
#my $switcher = $genenum / 2;
my $names = {};
open (T, ">$outdir/seq-gen.file");
for (my $g = 1; $g <= $genenum; $g++){
    my $treeindex = int (rand @trees);
    my $randtree  = $trees[$treeindex];
    my $randindex = int (rand @rates);
    my $randrate  = $rates[$randindex];
    my $string = $g . "_" . $treeindex . "_" . $randindex;
    $names->{$g} = $string;
#    my $randtree = $trees[int (rand @trees)];
#    my $randrate = $rates[int (rand @rates)];
    print T "[$genelen, $randrate]$randtree\n";
}
close (T);


#    if ($g <= $switcher){
#	print T "[$genelen, $rate1]$treestring\n";
#    }
#    else {
#	print T "[$genelen, $rate2]$treestring\n";
#    }
#}
#close (T);

# run seq-gen
my $len = $genelen * $genenum;
`seq-gen -mJTT -n1 -l$len -p$genenum < $outdir/seq-gen.file > $outdir/sim.phy`;

# parse to fasta                                                                                           
open (B, ">$outdir/sim.fa");                                                                                  
open my $xh, '<', "$outdir/sim.phy";                                                                          
my $phystring = <$xh>;                                                                                        
close $xh;                                                                                                    
my @phystring = split (/\n/, $phystring);                                                                     
shift @phystring;                                                                                            
...
#    my $filename;
#    if ($g <= $switcher){
#	$filename = $g . "_" . $rate1name;
#    }
#    else {
#	$filename = $g . "_" . $rate2name;
#    }

    my $filename = $names->{$g};
    open (A, ">$outdir/fastas/$filename");
    my $subseeker = ($g * $genelen) - $genelen;
    my $seqin = Bio::SeqIO->new (-format=>'Fasta', -file=>"$outdir/sim.fa");
    while (my $seq = $seqin->next_seq()){
	my $id        = $seq->display_id;
	my $sequence = $seq->seq;
	my $subseq = substr ($sequence, $subseeker, $genelen);
	print A ">$id\n$subseq\n";
    }
    close (A);
}
