...
=head1 SYNOPSIS                                                                   
  ...
  flocking_engine.pl --                                                           
    ...
    Borrowed heavily from codebase of 
    Victor L. Jacobs (www.dwepe.com/vljacobs/).
...
Options:                                                                          
 ...
 --threshold   Is the ILD value threshold beyond which only the 
                  separation parameter is in effect
 --type        Is the type of ILD value being compared: raw or pval
                  note raw only currently works for precomputed ILD raw stats
...
# TODO: 
# Take into account wrapping effect in three forces calc 
#    if we want to do wrap around (crawling around boundaries)
# jagged back and forth behavior when velocity limit set to 0.25?
...
my ($help, $matrix, $config, $outdir, $procs, $ildlist, $threshold, $highlights, $type);
GetOptions(
    'h|help'          => \$help,
    'm|matrix=s'      => \$matrix,
    'o|outdir=s'      => \$outdir,
...
for (my $it = 0; $it < $conf->{'ITERATIONS'}; $it++){
	...
	my @boidCells;
#	push (@boidCells, $boidIndex->{$counter});
...
#	for (my $counter2 = 0; $counter2 < $boidcount; $counter2++){
	foreach my $counter2 (@boidNabe){
		...
		if (distance (\@boidList, $counter2, $counter) < $conf->{'RADIUS'}){
			...
			$ildobj->generate_nxs_pairwise ($partitions->{$boidmap->{$counter}}, 
							...
							$boidmap->{$counter2},
							"$outdir/ild/nxs");
#			$ildobj->generate_ild_part ($party,
...
#			$pjobs->{$party} = $counterstring;
			...
			$icachecounter++;
#			$pjobscached->{$party} = $counterstring;
    ...
    $localtime = localtime;
    print STDERR "$it end nabe search $localtime\n";
    ...
    $localtime = localtime;
    print STDERR "$it end ILD search $localtime\n";    
	...
	my $string = $pjobscached->{$cachedjob};
        my ($c1, $c2) = split (/\:/, $string);
#	my ($pval, $slen, $ildchars) = $ildobj->parse_ild ("$outdir/ild/logs/$cachedjob.log");
...
#	print LOGIT "$counter\t$boidList[$counter]->[0]\t$boidList[$counter]->[1]\n";
...
#    `boids_gd.pl $outdir/logs/$it.log $conf->{'DIMMENSIONS'} $conf->{'DIMMENSIONS'} $highlights > $outdir/images/$it.png`;
#    `convert $outdir/images/$it.png GIF: > $outdir/images/$it.gif`;
    if ($highlights){
    ...
    print STDERR "ILDTEST\t$it\t$icounter\t$icachecounter\n";
...
# geneate the final animated gif using gifsicle
...
#    my $repelsepfactor = $config->{'SEPARATION_FACTOR'} * 2;
