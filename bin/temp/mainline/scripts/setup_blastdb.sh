...
if [[ $OID_USER_DIR == "" ]]; then
	print -u2 "OID_USER_DIR not defined ... exiting"
	exit 1
fi
...
if [[ -z $FORMATDB ]]; then
	...
	if [[ -d $BLASTDIR ]]; then
		print -u2 "BLAST installation not found!  Make sure BLASTHOME is defined in config file or BLAST executables are in your PATH."
		exit 1
	else
...
for i in "$@"; do
	...
	if [[ -s $i.psq ]]; then
		print "BLAST db file for $i already exists ... skipping"
		continue
	fi
	...
	if [[ -f $i.seq ]]; then
		...
		print -u2 "Fasta file for $i does not exist"
		exit 1
	fi

	# Check deflines
	if grep -q '^>.*[^A-Za-z0-9#._]' $fasta_file; then
		print -u2 "Error: Illegal characters in gene names in $fasta_file."
		print -u2 "Only letter, digits, ., and _ are allowed, with # being the species delimiter."
		exit 1
	fi
	if grep -q '^>[^#]*$' $fasta_file; then
		print -u2 "Warning: The following gene names do not contain a # character to delimit the species name and geneID:"
		grep '^>[^#]*$' $fasta_file
	fi
	
	# Create BLAST DB (assuming protein sequences)
	print "Creating ${i%%.*} BLAST db"
