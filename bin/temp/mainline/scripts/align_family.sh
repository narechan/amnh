...
if [[ ! -r $INPUT ]] ; then
	print -u2 "$INPUT does not exist or is not readable"
	exit 1
fi
if [[ -f $OUTPUT ]] ; then
	print -u2 "$OUTPUT exists"
	exit 1
fi

# Check MAFFT
if ! mafft /dev/null 2>/dev/null; then
	print -u2 "MAFFT is not installed or not found."
	exit 1
fi
