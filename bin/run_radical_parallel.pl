...
Options:
 ...
 --checking    Is set if you want to abort concatenation when the statistical criterion 
    ...
    must be specified.
--prefix       Is the SGE prefix for the run
...
my ($help, $matrixfile, $treefile, $outdir, $configfile, $step, $virt, $subx, $checking, $submit, $prefix);
GetOptions(
    'h|help'          => \$help,
...
for my $option ($matrixfile, $outdir, $configfile, $submit, $prefix){
...
if ($checking){
	...
	if ($submit == 1){
	    ...
	    print SHELL "time radical_run.pl -m $matrixfile -c $configfile -o $outdir -e $outdir/cmds/\$SGE_TASK_ID.$j.cmds\n";
	    ...
	    if ($virt){
		`qsub -l virtual_free=$virt -t 1-$samps:1 -r yes -N rad.$prefix.$j.sh -S /bin/bash -cwd -o $outdir/sge/stdout/$j.out -e $outdir/sge/stderr/$j.err $outdir/sge/scripts/$j.sh`;
	    }
	    else {
		`qsub -t 1-$samps:1 -r yes -N rad.$prefix.$j.sh -S /bin/bash -cwd -o $outdir/sge/stdout/$j.out -e $outdir/sge/stderr/$j.err $outdir/sge/scripts/$j.sh`;
	    }
	}
	
	# checking after every serial submit of a concatenation point
	# cannot serialize concat pts in each rep because testing a
	# given concat point
	elsif ($submit == 2){
	    open (SHELL, ">$outdir/sge/scripts/$j.sh");
	    print SHELL "#!/bin/bash\n";
	    ...
	    if ($virt){
		`qsub -l virtual_free=$virt -r yes -N rad.$prefix.$j.sh -S /bin/bash -cwd -o $outdir/sge/stdout/$j.out -e $outdir/sge/stderr/$j.err $outdir/sge/scripts/$j.sh`;
	    }
	    else {
		`qsub -r yes -N rad.$prefix.$j.sh -S /bin/bash -cwd -o $outdir/sge/stdout/$j.out -e $outdir/sge/stderr/$j.err $outdir/sge/scripts/$j.sh`;
	    }
	}
	else {
	...
	# submit the checker
	open (C, ">$outdir/sge/scripts/checker.$j.sh");
	print C "#!/bin/bash\n";
	print C "date\n";
	print C "source ~/.bashrc\n";
	print C "radical_check.pl -m $matrixfile -t $treefile -c $configfile -o $outdir -p $j\n";
	print C "hostname\n";
	print C "date\n";
	print C "touch $outdir/lock.$j\n";
	close (C);

	print STDERR "Submitting checker.$j.sh\n";
	`qsub -hold_jid rad.$prefix.* -r yes -N checker.$j.sh -S /bin/bash -cwd -o $outdir/sge/stdout/checker.$j.out -e $outdir/sge/stderr/checker.$j.err $outdir/sge/scripts/checker.$j.sh`;
	
	# hold until lock file appears for this iteration
	my $bail = 0;
	until ($bail > 0){
	    if (-e "$outdir/lock.$j"){
		$bail++;
    ...
    # getting all the concat sizes in each replicatesl -- hence the flip
    elsif ($submit == 2){
	foreach my $i (sort {$a <=> $b} keys %samps){
	    ...
	    if ($virt){
		`qsub -l virtual_free=$virt -r yes -N rad.$prefix.$i.sh -S /bin/bash -cwd -o $outdir/sge/stdout/$i.out -e $outdir/sge/stderr/$i.err $outdir/sge/scripts/$i.sh`;
	    }
	    else {
		`qsub -r yes -N rad.$prefix.$i.sh -S /bin/bash -cwd -o $outdir/sge/stdout/$i.out -e $outdir/sge/stderr/$i.err $outdir/sge/scripts/$i.sh`;
...
foreach my $j (sort {$a <=> $b} keys %$concats){
    ...
    `qsub -hold_jid rad.$prefix.* -r yes -N topos.$j.sh -S /bin/bash -cwd -o $outdir/sge/stdout/topos.$j.out -e $outdir/sge/stderr/topos.$j.err $outdir/sge/scripts/topos.$j.sh`;
}
