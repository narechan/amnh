...
my ($help, $matrix, $config, $outdir, $procs, $ildlist, $highlights, $charfile);
GetOptions(
    'h|help'          => \$help,
    'm|matrix=s'      => \$matrix,
    'o|outdir=s'      => \$outdir,
...
#    my $repelboids   = {};
	...
	$ilds->{$c1}->{$c2} = $pval;
	$ilds->{$c2}->{$c1} = $pval;
	$boidstocheck->{$c1}->{$c2} = 1;
    }
    
    # update the boids to check with stuff that's in the cache
...
sub separation{
    ...
    foreach my $ctr2 (keys %$boidstocheck){
	if ((distance ($boidList, $ctr2, $ctr1) <= $config->{'RADIUS'}) and (distance ($boidList, $ctr2, $ctr1) > $config->{'SEPARATION_FACTOR'})){
	    $tmpV[0] = $tmpV[0] - (($boidList->[$ctr2]->[0] - $boidList->[$ctr1]->[0]) * $ilds->{$ctr1}->{$ctr2});
            $tmpV[1] = $tmpV[1] - (($boidList->[$ctr2]->[1] - $boidList->[$ctr1]->[1]) * $ilds->{$ctr1}->{$ctr2});
	}
	elsif (distance ($boidList, $ctr2, $ctr1) <= $config->{'SEPARATION_FACTOR'}){
