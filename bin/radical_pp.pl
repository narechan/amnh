...
# get the treefile name for holding dirs
my $treefile_name;
if ($treefile =~/\//g){
    $treefile =~m/.*\/(.*)$/;
    $treefile_name = $1;
...
# print out annotated tree with fixation pts and aucs
...
#				 -format => 'nexus');
    ...
    # cycle through internal nodes and assign values
...
#	(push @asym, $y) if ($include == 1);
    ...
    # take the first value that has data for the starting position:
    # work around for the case where missing data prevents tree
    # reconstruction at the early stages
    push (@asym, $y[0]);
