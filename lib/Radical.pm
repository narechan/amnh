...
sub run_tnt{
    ...
    my @chunk = split (/\./, $filename);
    
    my $cmd = "tnt ";
    
    # note that we run the strict (nelsen) consensus 
    # every time in case there's more than one most 
    # parsimonious tree
    $cmd .= "bground mxram 2000, p $file, echo=, log $outdir/logs/$chunk[0].$chunk[1].log, $self->{'configuration'}->{'TREECOMMAND'}, taxname=, nelsen *, tsave *$outdir/trees/$chunk[0].$chunk[1].tre, save, le, quit,";
    `$cmd`;
}
...
sub generate_cmds{
	...
	foreach my $number (@dothese){
	    ...
	    # skip the rest if we're in RAXML or TNT  mode
	    if (($mode eq "RAXML") or ($mode eq "TNT")){
...
#    print CAT "FORMAT INTERLEAVE SYMBOLS=\"ABCDEFGHIKLMNPQRSTUVWXYZ\" DATATYPE=PROTEIN MISSING=? GAP=-;\n";
    print CAT "FORMAT INTERLEAVE DATATYPE=$datatype MISSING=? GAP=-;\n";
    print CAT "MATRIX\n";
...
sub get_datatype{
    my $self = shift;
    return $self->{'alignment'}->{'datatype'};
}
